...
public class HadoopWordPairs extends Configured implements Tool {
	...
	public static class Map extends Mapper<LongWritable, Text, Text, IntWritable> {
		...
		public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			String[] splitLine = value.toString().toLowerCase().split(Utils.WORD_SEPARATOR);

			for (String token : splitLine) {
				if (Utils.WORD_PATTERN.matcher(token).matches() || (Utils.NUMBER_PATTERN.matcher(token).matches() && token.length() >= 4 && token.length() <= 16)) {
